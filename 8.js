...
function defaultUpdate() {
 editables.forEach(function(editable) {
   ...
   editable.style.fontSize = 35 * neutraliser + "px";
   editable.style.lineHeight = 35 * neutraliser + "px";
   editable.style.color = "white";
   editable.style.textAlign = "center";
 });
 heading.style.fontFamily = "Pacifico";
 heading.style.fontSize = 40 * neutraliser + "px";
  heading.style.lineHeight = 40 * neutraliser + "px";
...
const logBtn = document.getElementById('btnLogOut');
firebase.auth().onAuthStateChanged(function(user) {
 if (user) {
   ...
   logBtn.style.display = "block";
   logBtn.href = "https://www.asobi.com.au/signin";
 }
});
...
//LogOut
  logBtn.addEventListener("click", function (){
    firebase.auth().signOut().then(function (){
      window.location.assign("https://www.asobi.com.au");
    });
  });
...
if (!queryString || urlParams.has('ghostUser')) {
  if(!queryString){
    defaultUpdate();
    console.log('default');
  } else {
    db.collection("ghostUsers").doc(urlParams.get('ghostUser')).get()
      .then(function(data) {
        data.data().updateInfo.forEach(function(value, index) {
          ...
          currentEditable.style.fontSize = value.fontSize * neutraliser + "px";
          currentEditable.style.lineHeight = value.fontSize * neutraliser + "px";
   ...
   if(isLoggedIn){
     ...
     editables.forEach(function(editable) {
       let data = {
         ...
         fontSize: editable.style.fontSize.replace(/px/, "") / neutraliser,
     ...
     db.collection("ghostUsers").add({
       ...
       .then(function(docRef) {
         window.location.assign("https://asobi.com.au/signin/?redirect="+window.location.href+'?ghostUser=' + docRef.id);
       });
   }
 });
} else {
...
successPopUp.addEventListener("click", function() {
 ...
 setTimeout(function(){
   document.querySelector('.also-try').style.transform = "scale(1, 1) translate(-50%, -50%)";
 }, 300);
});
...
document.querySelector('.no-thanks').addEventListener("click", function(){
  document.querySelector('.also-try').style.transform = "scale(0, 0) translate(-50%, -50%)";
});
